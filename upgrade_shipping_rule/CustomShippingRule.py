...
class CustomShippingRule(Document):
	...
	def apply(self, doc):
		...
		quantity_shipping_rule = frappe.get_doc("Shipping Rule Settings")
		is_it_item_based=False
		price_per_item=0
		excluded_list=[]
		for excluded_city in quantity_shipping_rule.exclude_cities:
			if( excluded_city.shipping_rule== self.name):
				excluded_list.append(excluded_city.city)
		qty=0
		for item in doc.items:
			qty=qty+item.qty
		for item_quantity in quantity_shipping_rule.quantity_based_shipping_for_shipping_rules:
			if(item_quantity.shipping_rule == self.name):
				is_it_item_based=True
				if( qty>= item_quantity.range_from and qty<= item_quantity.range_to):
					price_per_item=item_quantity.price_per_item
				if( qty>= item_quantity.range_from and item_quantity.range_to<=0 ) :
					price_per_item=item_quantity.price_per_item
		...
		if doc.get_shipping_address():
			if doc.get_shipping_address().get("city") in excluded_list:
				#throw error here
				frappe.throw(
					_("Shipping Address city is not available for this Shipping Rule")
				)
		...
		#Customized code to calculate based on item quantity
		if is_it_item_based:
			if( price_per_item >0  ):
				shipping_amount = qty * price_per_item
